...
def resize_image(
    ...
    pil_image = Image.fromarray(image if image.dtype == np.uint8 else image.astype(np.uint8))
    pil_image = pil_image.resize((w_new, h_new), Image.BILINEAR if interp != "nearest" else Image.NEAREST)
